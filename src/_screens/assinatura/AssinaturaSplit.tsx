...
import { SaveSplit,  detailstAssinatura,
  listAssociadosSplit, listExtratoSplit  } from '@/lib/api/_assinatura';
...
export default function AssinaturaSplit({ idassinatura }: Props) {
...
//listExtrato
  ...
  const resultExtrato = await listExtratoSplit(idassinatura);


    if( resultExtrato.event === 'EXTRATO_LIST_SUCCESS' ){


    console.log('******************* resultExtrato')
    console.log( resultExtrato )
    console.log('******************* resultExtrato')
...
const syncDetails = async () =>{
  const resultDetails = await detailstAssinatura(idassinatura);
  const  dataDetails = resultDetails.result;




  if( resultDetails.event ==='ASSINATURA_DETAILS_SUCCESS'){
    //toast.success(resultDetails.message);
    setdetailsAssinatura(dataDetails);
  }

  if( resultDetails.event ==='ASSINATURA_DETAILS_FAILED'){
    toast.info(resultDetails.message);
  }

}

const syncAssociadosSplitList = async () =>{
  const responseAssociadosSplitList = await listAssociadosSplit(idassinatura);
  ...
  const settingSplit = async () => {
    ...
    try {
      //setLoading(true);
          const response = await SaveSplit(dadosSaveSplit);
          ...
          <div className="basis-full md:basis-3/4 w-full max-w-full border border-gray-200 shadow-sm p-4">
            ...
            {datalistExtratoSplit.map((dataExtrato) => (
              <TableRow key={dataExtrato.id_extrato}>
                ...
                {/* <TableCell ><span className="font-medium" >{dataExtrato.status}</span></TableCell> */}
